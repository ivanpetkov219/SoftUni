...
public class BookServiceImpl implements BookService {
    ...
    @Override
    public List<Book> getAllBooksByEditionTypeAndLessThan5000Copies(EditionType editionType, int copies) {
        return this.bookRepository.findAllByEditionTypeAndCopiesLessThan(editionType, copies);
    }

    @Override
    public List<Book> getAllBooksByPriceLowerAndHigherThanGiven(BigDecimal lower, BigDecimal greater) {
        return this.bookRepository.findAllByPriceLessThanOrPriceGreaterThan(lower, greater);
    }

    @Override
    public List<Book> getAllBooksThatAreNotInGivenInput(LocalDate before, LocalDate after) {
        return this.bookRepository.findAllByReleaseDateBeforeAndReleaseDateAfter(before, after);
    }

    @Override
    public List<Book> getAllBooksByReleaseDateBeforeGiven(LocalDate before) {
        return this.bookRepository.findBookByReleaseDateBefore(before);
    }

    @Override
    public List<Book> getAllBooksThatContainsGivenPattern(String pattern) {
        return this.bookRepository.findAllByTitleContains(pattern);
    }

    @Override
    public List<Book> getAllBooksByAuthorLastNameContainsGivenStringPattern(String pattern) {
        String patternPlusDelimiter = pattern + "%";
        return this.bookRepository.findAllBooksWhereAuthorLastNameContainsGivenPattern(patternPlusDelimiter);
    }

    @Override
    public List<Book> getAllBooksByGivenCharacterLength(int length) {
        return this.bookRepository.findCountOfBooksByGivenCharLength(length);
    }

    @Override
    public List<Book> getBooksByGivenAuthorName(String name) {
        return this.bookRepository.findAllBooksByGivenAuthor(name);
    }
