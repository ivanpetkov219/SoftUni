...
public class Spaceship {
    ...
    public void add(Astronaut astronaut) {
        if (astronauts.size() == this.getCapacity()) {
            throw new IllegalArgumentException(SPACESHIP_FULL);     //DONE
        }
        ...
        if (astronautExists) {
            throw new IllegalArgumentException(String.format(ASTRONAUT_EXIST, astronaut.getName()));     //DONE
        }

        this.astronauts.add(astronaut);     //DONE
    }
    ...
    private void setCapacity(int capacity) {
        if (capacity < ZERO_CAPACITY) {
            throw new IllegalArgumentException(INVALID_CAPACITY);     //DONE
        }
