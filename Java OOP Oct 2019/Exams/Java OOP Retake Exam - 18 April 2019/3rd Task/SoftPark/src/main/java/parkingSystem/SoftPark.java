...
public class SoftPark {
    ...
    public Map<String, Car> getParking() {
        return Collections.unmodifiableMap(this.parking);        //TODO -> check
    }

    public String parkCar(String parkSpot, Car car) {
        if (!this.parking.containsKey(parkSpot)) {
            throw new IllegalArgumentException("Parking spot doesn't exists!");        //TODO -> check
        }

        if (this.parking.get(parkSpot) != null) {
            throw new IllegalArgumentException("Parking spot is already taken!");        //TODO -> check
        }
       ...
       Car optionalCar =this.parking.values().stream().filter(c->c!=null).findAny().orElse(null);
        ...
        if (carExists) {
            throw new IllegalStateException("Car is already parked!");        //TODO -> check
        }

        this.parking.put(parkSpot,car);

        return String.format("Car:%s parked successfully!",car.getRegistrationNumber());        //TODO -> check
    }
    ...
    public String removeCar(String parkSpot, Car car) {
        ...
        if (this.parking.get(parkSpot) != car) {
            throw new IllegalArgumentException("Car for that spot doesn't exists!");        //TODO -> check
        }

        this.parking.put(parkSpot,null);

        return String.format("Remove car:%s successfully!",car.getRegistrationNumber());        //TODO -> check
    }
}
